"""
Project: ICEBERG middleware Project
...
import mock
import pandas as pd
from iceberg.discovery import image_discovery

# ------------------------------------------------------------------------------
#
@mock.patch('iceberg.discovery.image_disc.glob',
...
def test_image_discovery_2():
    ...
    image_discovery(path='./', filename='test')
...
@mock.patch('iceberg.discovery.image_disc.glob',
            return_value=['test1.tif', 'test2.tif'])
@mock.patch('iceberg.discovery.image_disc.os.path.getsize',
